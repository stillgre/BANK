create table bal_report as (
    select 
    q1.client_id, 
    current_amt, 
    trans_bal,
   CASE
    when current_amt = trans_bal then 'балансы совпадают'
    else 'наблюдается рассинхрон'
    end as comment
    from (select sum(trans_amt) as trans_bal, cl.client_id from transactions as t
